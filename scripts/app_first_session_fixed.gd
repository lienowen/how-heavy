...
const FirstSessionWorld = preload("res://scripts/world_first_session.gd")

func _ready() -> void:
	web_audio_armed = not OS.has_feature("web")
	if Game.progress.completed.is_empty() and int(Game.progress.unlocked_level) <= 1:
		launch_level(1)
	else:
		show_title()
